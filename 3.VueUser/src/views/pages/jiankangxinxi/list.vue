	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"#fff","width":"80%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('jiankangxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
			...
			<div class="data_box">
				<div class="data_view">
					<el-table v-loading="listLoading" class="data_table" :data="list" border :row-style="{'cursor':'pointer'}"
						...
						<el-table-column label="慢性病" :resizable='true' align="left" header-align="left">
							<template #default="scope">
								{{scope.row.manxingbing}}
							</template>
						</el-table-column>
						<el-table-column label="家族病史" :resizable='true' align="left" header-align="left">
							<template #default="scope">
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/jiankangxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const tableDetailClick = (row) => {
		router.push('jiankangxinxiDetail?id=' + row.id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
