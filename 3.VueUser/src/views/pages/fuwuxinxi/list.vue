	...
	<div class="app-contain" :style='{"minHeight":"100vh","padding":"0 0 40px","margin":"0 auto","overflow":"hidden","borderRadius":"4px","background":"#fff","width":"80%","position":"relative"}'>
		...
		<el-form :inline="true" :model="searchQuery" class="list_search">
			...
			<div class="search_btn_view">
				...
				<el-button class="add_btn" type="success" v-if="btnAuth('fuwuxinxi','新增')" @click="addClick">新增</el-button>
			</div>
		</el-form>
					...
					<mySwiper :data="list" :type="3"
						...
						<template #default="scope">
							<div class="data_item animation_box" @click.stop="detailClick(scope.row.id)" >
								<div class="data_img_box" v-if="scope.row.tupian && scope.row.tupian.substr(0,4)=='http'" @click.stop="preViewClick(scope.row.tupian)">
									<el-image class="data_img" :src="scope.row.tupian" fit="cover"></el-image>
								</div>
								<div class="data_img_box" v-else @click.stop="preViewClick($config.url+scope.row.tupian.split(',')[0])">
									<el-image class="data_img" :src="scope.row.tupian?$config.url + scope.row.tupian.split(',')[0]:''"
										fit="cover"></el-image>
								</div>
								<div class="data_content">
									<div class="data_title">服务名称：{{scope.row.fuwumingcheng}}</div>
									<div class="data_title">{{scope.row.fuwushijian}}</div>
...
<script setup>
	...
	const addClick = () => {
		router.push('/index/fuwuxinxiAdd')
	}
	//判断是否从个人中心跳转
	...
	const getList = () => {
		...
		if (categoryIndex.value != -1) {
			params.fuwuleixing = categoryList.value[categoryIndex.value]
		}
	...
	const detailClick = (id) => {
		router.push('fuwuxinxiDetail?id=' + id + (centerType.value?'&&centerType=1':''))
	}
	//下载文件
	const download = (file) =>{
		if(!file){
