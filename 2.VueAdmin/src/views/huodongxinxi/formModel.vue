		...
		<el-dialog v-model="formVisible" :title="formTitle" width="50%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="活动区域" prop="huodongquyu">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.huodongquyu?true:false"
								v-model="form.huodongquyu" 
								placeholder="请选择活动区域"
								>
								<el-option v-for="(item,index) in huodongquyuLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.huodongneirong = res.data.data.huodongneirong?(res.data.data.huodongneirong.replace(reg,'../../../cl3973433/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='huodongquyu'){
					form.value.huodongquyu = row[x];
					disabledForm.value.huodongquyu = true;
					continue;
				}
				if(x=='huodongdidian'){
		...
		context?.$http({
			...
			huodongquyuLists.value = res.data.data
		})
	}
	//初始化
	//声明父级调用
